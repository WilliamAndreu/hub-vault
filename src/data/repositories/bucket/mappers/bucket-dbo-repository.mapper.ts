...
import { BucketDBO } from '@data/datasource/bucket/local/dbo/bucket.dbo';
import { BucketEntity } from '@models/bucket/bucket-entity';



export class BucketDboRepositoryMapper extends Mapper<
BucketDBO,
  BucketEntity
> {


  mapTo(param: BucketDBO): BucketEntity {
    return {
      id: param.id,
  ...
  mapFrom(param: BucketEntity): BucketDBO {
    return {
      id: param.id,
      node_id: param.node_id,
      name: param.name,
